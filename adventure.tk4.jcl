...
1
 ...
 ADVENT: PROC OPTIONS (MAIN);
       ...
       DCL ITIME ENTRY (FIXED BIN(31));

       DCL TOTING ENTRY (FIXED BIN(31)) RETURNS (BIT(1));

       DCL HERE ENTRY (FIXED BIN(31)) RETURNS (BIT(1));

       DCL AT ENTRY (FIXED BIN(31)) RETURNS (BIT(1));

       DCL LIQ2 ENTRY (FIXED BIN(31)) RETURNS (FIXED BIN(31));

       DCL LIQ ENTRY (FIXED BIN(31)) RETURNS (FIXED BIN(31));

       DCL LIQLOC ENTRY (FIXED BIN(31)) RETURNS (FIXED BIN(31));

       DCL BITSET ENTRY (FIXED BIN(31),FIXED BIN(31))
                  RETURNS (BIT(1));

       DCL FORCED ENTRY (FIXED BIN(31)) RETURNS (BIT(1));

       DCL DARK ENTRY (FIXED BIN(31)) RETURNS (BIT(1));

       DCL PCT ENTRY (FIXED BIN(31)) RETURNS (BIT(1));

       DCL SPEAK ENTRY (FIXED BIN(31));

       DCL PSPEAK ENTRY (FIXED BIN(31),FIXED BIN(31));

       DCL RSPEAK ENTRY (FIXED BIN(31));

       DCL GETIN ENTRY (CHAR(5),CHAR(5),CHAR(5),CHAR(5));

       DCL YES ENTRY (FIXED BIN(31),FIXED BIN(31),FIXED BIN(31))
               RETURNS (BIT(1));

       DCL A5TOA1 ENTRY (CHAR(5),CHAR(5),(10) CHAR(1),FIXED BIN(31));

       DCL VOCAB ENTRY (CHAR(5),FIXED BIN(31))
                 RETURNS (FIXED BIN(31));

       DCL DSTROY ENTRY (FIXED BIN(31));

       DCL JUGGLE ENTRY (FIXED BIN(31));

       DCL MOVE ENTRY (FIXED BIN(31),FIXED BIN(31));

       DCL PUT ENTRY (FIXED BIN(31),FIXED BIN(31),FIXED BIN(31))
               RETURNS (FIXED BIN(31));

       DCL CARRY ENTRY (FIXED BIN(31),FIXED BIN(31));

       DCL DROP ENTRY (FIXED BIN(31),FIXED BIN(31));

       DCL BUG ENTRY (FIXED BIN(31));

       DCL RAN ENTRY (FIXED BIN(31)) RETURNS (FIXED BIN(31));
         ...
         DCL 1 VOCCOM STATIC,
               2 KTAB(320) FIXED BIN(31),
               2 ATAB(320) CHAR(5),
               2 TABSIZ FIXED BIN(31) INIT(320);
...
1  DCL 1  ADVARS STATIC,
       ...
       2  SELECT       FIXED BIN(31)  INIT(0),
       2  SETUP        FIXED BIN(31)  INIT(0),
...
1/*  READ THE DATABASE IF WE HAVE NOT YET DONE SO       */
 ...
 L1000:  PUT STRING (OUTSTR) EDIT ('Initializing...') (A);
         CALL LINEOUT;
 ...
 */
 ...
 L1002:  GET FILE (CAVES) EDIT (SECT) (COL(1),F(8));
         ...
         CALL LINEOUT;
         SELECT  = (SECT);
            IF SELECT = 00 THEN GOTO L1100;
            IF SELECT = 01 THEN GOTO L1004;
            IF SELECT = 02 THEN GOTO L1004;
            IF SELECT = 03 THEN GOTO L1030;
            IF SELECT = 04 THEN GOTO L1040;
            IF SELECT = 05 THEN GOTO L1004;
            IF SELECT = 06 THEN GOTO L1004;
            IF SELECT = 07 THEN GOTO L1050;
            IF SELECT = 08 THEN GOTO L1060;
            IF SELECT = 09 THEN GOTO L1070;
            IF SELECT = 10 THEN GOTO L1004;
            IF SELECT = 11 THEN GOTO L1080;
         CALL BUG(9);
 ...
 */
 ...
 L2630:  I=VOCAB(WD1,-1);
         ...
         KQ=I/1000+1;
         SELECT = (KQ-1);
            IF SELECT = 00 THEN GOTO L8;
            IF SELECT = 01 THEN GOTO L5000;
            IF SELECT = 02 THEN GOTO L4000;
            IF SELECT = 03 THEN GOTO L2010;
         CALL BUG(22);
 ...
 */
 ...
 L4080:  SELECT = (VERB);
            IF SELECT = 01 THEN GOTO L8010;       /* TAKE */
            IF SELECT = 02 THEN GOTO L8000;       /* DROP */
            IF SELECT = 03 THEN GOTO L8000;       /* SAY */
            IF SELECT = 04 THEN GOTO L8040;       /* OPEN */
            IF SELECT = 05 THEN GOTO L2009;       /* NOTH */
            IF SELECT = 06 THEN GOTO L8040;       /* LOCK */
            IF SELECT = 07 THEN GOTO L9070;       /* ON */
            IF SELECT = 08 THEN GOTO L9080;       /* OFF */
            IF SELECT = 09 THEN GOTO L8000;       /* WAVE */
            IF SELECT = 10 THEN GOTO L8000;       /* CALM */
            IF SELECT = 11 THEN GOTO L2011;       /* WALK */
            IF SELECT = 12 THEN GOTO L9120;       /* KILL */
            IF SELECT = 13 THEN GOTO L9130;       /* POUR */
            IF SELECT = 14 THEN GOTO L8140;       /* EAT */
            IF SELECT = 15 THEN GOTO L9150;       /* DRNK */
            IF SELECT = 16 THEN GOTO L8000;       /* RUB */
            IF SELECT = 17 THEN GOTO L8000;       /* TOSS */
            IF SELECT = 18 THEN GOTO L8180;       /* QUIT */
            IF SELECT = 19 THEN GOTO L8000;       /* FIND */
            IF SELECT = 20 THEN GOTO L8200;       /* INVN */
            IF SELECT = 21 THEN GOTO L8000;       /* FEED */
            IF SELECT = 22 THEN GOTO L9220;       /* FILL */
            IF SELECT = 23 THEN GOTO L9230;       /* BLST */
            IF SELECT = 24 THEN GOTO L8240;       /* SCOR */
            IF SELECT = 25 THEN GOTO L8250;       /* FOO */
            IF SELECT = 26 THEN GOTO L8260;       /* BRF */
            IF SELECT = 27 THEN GOTO L8270;       /* READ */
            IF SELECT = 28 THEN GOTO L8000;       /* BREK */
            IF SELECT = 29 THEN GOTO L8000;       /* WAKE */
            IF SELECT = 30 THEN GOTO L8300;       /* SUSP */
            IF SELECT = 31 THEN GOTO L8310;       /* HOUR */
            IF SELECT = 32 THEN GOTO SETLOG;      /* LOG  */
 ...
 L4090:  SELECT = (VERB);
            IF SELECT = 01 THEN GOTO L9010;       /* TAKE */
            IF SELECT = 02 THEN GOTO L9020;       /* DROP */
            IF SELECT = 03 THEN GOTO L9030;       /* SAY */
            IF SELECT = 04 THEN GOTO L9040;       /* OPEN */
            IF SELECT = 05 THEN GOTO L2009;       /* NOTH */
            IF SELECT = 06 THEN GOTO L9040;       /* LOCK */
            IF SELECT = 07 THEN GOTO L9070;       /* ON */
            IF SELECT = 08 THEN GOTO L9080;       /* OFF */
            IF SELECT = 09 THEN GOTO L9090;       /* WAVE */
            IF SELECT = 10 THEN GOTO L2011;       /* CALM */
            IF SELECT = 11 THEN GOTO L2011;       /* WALK */
            IF SELECT = 12 THEN GOTO L9120;       /* KILL */
            IF SELECT = 13 THEN GOTO L9130;       /* POUR */
            IF SELECT = 14 THEN GOTO L9140;       /* EAT */
            IF SELECT = 15 THEN GOTO L9150;       /* DRNK */
            IF SELECT = 16 THEN GOTO L9160;       /* RUB */
            IF SELECT = 17 THEN GOTO L9170;       /* TOSS */
            IF SELECT = 18 THEN GOTO L2011;       /* QUIT */
            IF SELECT = 19 THEN GOTO L9190;       /* FIND */
            IF SELECT = 20 THEN GOTO L9190;       /* INVN */
            IF SELECT = 21 THEN GOTO L9210;       /* FEED */
            IF SELECT = 22 THEN GOTO L9220;       /* FILL */
            IF SELECT = 23 THEN GOTO L9230;       /* BLST */
            IF SELECT = 24 THEN GOTO L2011;       /* SCOR */
            IF SELECT = 25 THEN GOTO L2011;       /* FOO */
            IF SELECT = 26 THEN GOTO L2011;       /* BRF */
            IF SELECT = 27 THEN GOTO L9270;       /* READ */
            IF SELECT = 28 THEN GOTO L9280;       /* BREK */
            IF SELECT = 29 THEN GOTO L9290;       /* WAKE */
            IF SELECT = 30 THEN GOTO L2011;       /* SUSP */
            IF SELECT = 31 THEN GOTO L2011;       /* HOUR */
            IF SELECT = 32 THEN GOTO L2011;       /* LOG  */
         CALL BUG(24);
 ...
 */
 ...
 L30000: NEWLOC=NEWLOC-300;
         SELECT = (NEWLOC-1);
             IF SELECT = 00 THEN GOTO L30100;
             IF SELECT = 01 THEN GOTO L30200;
             IF SELECT = 02 THEN GOTO L30300;
         CALL BUG(20);
 ...
 */
 ...
 L8300:  CALL RSPEAK(201);
         GOTO L2000;

 /* HOURS.  REPORT CURRENT NON-PRIME-TIME HOURS. */

 L8310:  CALL LINESKP;
         PUT STRING (OUTSTR) EDIT ('Open all day!') (A); 
         CALL LINEOUT;
         GOTO L2012;
 ...
 */
 ...
 L40000: SELECT = (HINT-4);
             IF SELECT = 00 THEN GOTO L40400;        /* CAVE */
             IF SELECT = 01 THEN GOTO L40500;        /* BIRD */
             IF SELECT = 02 THEN GOTO L40600;        /* SNAKE */
             IF SELECT = 03 THEN GOTO L40700;        /* MAZE */
             IF SELECT = 04 THEN GOTO L40800;        /* DARK */
             IF SELECT = 05 THEN GOTO L40900;        /* WITT */
         CALL BUG(27);
...
//SYSUT1   DD  *
./ ADD NAME=ADVENT
ALLOC FI(CAVES) DA('SYSGEN.ADVENT(CAVES)') SHR REUSE
